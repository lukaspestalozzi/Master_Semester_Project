...
class Ismcts(object):
    ...
    **Selection:** UCB1
    ...
    def search(self, root_state: TichuState, observer_id: int, iterations: int, cheat: bool = False, clear_graph: bool=True, max_time: int=float('inf')) -> PlayerAction:
        """
        ...
        iteration = 0
        while iteration < iterations or time() > end_t:
            iteration += 1
            self.init_iteration()
            # logger.debug("iteration "+str(iteration))
            state_det = root_search_state if cheat else self.determine(state=root_search_state, observer=self.observer_id)
    ...
    @abc.abstractmethod
    def determine(self, state: TichuState, observer: int)->TichuState:
...
class RandomDeterminePolicy(DeterminePolicy, metaclass=abc.ABCMeta):
    ...
    def determine(self, state: TichuState, observer: int) -> TichuState:
        return Determiner(state, observer).random_determinization()


class PoolDeterminePolicy(DeterminePolicy, metaclass=abc.ABCMeta):

    def determine(self, state: TichuState, observer: int) -> TichuState:
        return Determiner(state, observer)._pool_strategy()


class SingleDeterminePolicy(DeterminePolicy, metaclass=abc.ABCMeta):

    def determine(self, state: TichuState, observer: int) -> TichuState:
        return Determiner(state, observer)._single_card_strategy()
...
class Determiner(object):
    ...
    def random_determinization(self):
        unknown_cards = list(self._unknown_cards())
        ...
        return self._apply_the_determinization_to_state(new_hc_list)
    ...
    def _single_card_strategy(self)->TichuState:
        ...
        remaining_cards_gcomb_dict: Dict[Card, GeneralCombination] = {c: GeneralCombination.from_combination(Single(c)) for c in self._cards}

        assert len(remaining_cards_gcomb_dict) == sum(self._pos_to_goallength.values())  # cards to distribute match the goal lengths
        ...
        assert len(remaining_cards_gcomb_dict) == self._pos_to_goallength[last_ppos], "{}, {}".format(len(remaining_cards_gcomb_dict), self._pos_to_goallength[last_ppos])
        assert len(handcards[last_ppos]) == 0
